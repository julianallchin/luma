...
You build with verbs, not coordinates. `luma.venue.catalog()` is the placeable vocabulary — every piece and the sockets it offers. `place`, `attach`, `extend`, `duplicate`, `detach`, `remove`, `trim` and `distribute` change the rig; `distribute` is the only way a fixture is ever created. Every one of them hands back a report whose `describe()` is the whole tree as it now stands, so read that rather than guessing. `luma.venue.describe()`, `luma.venue.dangling()` and `luma.venue.unplaced()` read the room live; `luma.venue.tiles()` draws it from above. A refusal raises `luma.VenueRefused`, and its message is the fix — there are only two: a socket pair the catalog forbids, and an extend longer than the gap it measured.
