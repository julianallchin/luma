...
pub async fn get_patch_hierarchy_pool(
...
pub async fn patch_fixture_pool(
    pool: &sqlx::SqlitePool,
    app: &AppHandle,
    venue_id: i64,
    universe: i64,
...
pub async fn get_patched_fixtures(
    ...
    get_patched_fixtures_pool(&db.0, venue_id).await
}

pub async fn get_patched_fixtures_pool(
    pool: &sqlx::SqlitePool,
    venue_id: i64,
) -> Result<Vec<PatchedFixture>, String> {
    let fixtures = sqlx::query_as::<_, PatchedFixture>(
...
pub async fn move_patched_fixture(
    ...
    move_patched_fixture_pool(&db.0, &app, venue_id, id, address).await
}

pub async fn move_patched_fixture_pool(
    pool: &sqlx::SqlitePool,
    app: &AppHandle,
    venue_id: i64,
    id: String,
...
pub async fn move_patched_fixture_spatial(
    ...
    move_patched_fixture_spatial_pool(
        &db.0, &app, venue_id, id, pos_x, pos_y, pos_z, rot_x, rot_y, rot_z,
    )
    .await
}

pub async fn move_patched_fixture_spatial_pool(
    pool: &sqlx::SqlitePool,
    app: &AppHandle,
    venue_id: i64,
    id: String,
...
pub async fn remove_patched_fixture(
    ...
    remove_patched_fixture_pool(&db.0, &app, venue_id, id).await
}

pub async fn remove_patched_fixture_pool(
    pool: &sqlx::SqlitePool,
    app: &AppHandle,
    venue_id: i64,
    id: String,
...
pub async fn rename_patched_fixture(
    ...
    rename_patched_fixture_pool(&db.0, &app, venue_id, id, label).await
}

pub async fn rename_patched_fixture_pool(
    pool: &sqlx::SqlitePool,
    app: &AppHandle,
    venue_id: i64,
    id: String,
...
// ============================================================================
// Database helper functions (used by other modules like schema.rs)
// ============================================================================

/// Get all fixtures from the database (no venue filter)
/// Used by graph execution nodes that need fixture data
pub async fn get_all_fixtures(pool: &sqlx::SqlitePool) -> Result<Vec<PatchedFixture>, String> {
    let fixtures = sqlx::query_as::<_, PatchedFixture>(
        "SELECT id, universe, address, num_channels, manufacturer, model, mode_name, fixture_path, label, pos_x, pos_y, pos_z, rot_x, rot_y, rot_z
         FROM fixtures"
    )
