...
/// Core: get waveform data for a track
pub async fn get_track_waveform_pool(
    pool: &SqlitePool,
    track_id: i64,
) -> Result<TrackWaveform, String> {
    let duration_seconds = crate::database::local::tracks::get_track_duration(pool, track_id)
        .await?
