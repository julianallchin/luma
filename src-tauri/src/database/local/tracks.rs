...
pub async fn list_tracks_pool(pool: &SqlitePool) -> Result<Vec<TrackSummary>, String> {
...
pub async fn list_tracks(db: State<'_, Db>) -> Result<Vec<TrackSummary>, String> {
    list_tracks_pool(&db.0).await
}

pub async fn import_track_pool(
    pool: &SqlitePool,
...
pub async fn import_track(
    ...
    import_track_pool(&db.0, app_handle, file_path).await
}

pub async fn get_melspec_pool(
    pool: &SqlitePool,
    fft_service: State<'_, crate::audio::FftService>,
    track_id: i64,
) -> Result<MelSpec, String> {
    let (file_path, track_hash) = get_track_path_and_hash(pool, track_id)
        .await
...
pub async fn get_melspec(
    ...
    get_melspec_pool(&db.0, fft_service, track_id).await
}

/// Core: get beat grid data for a track
pub async fn get_track_beats_pool(
    pool: &SqlitePool,
    track_id: i64,
) -> Result<Option<BeatGrid>, String> {
    let row = get_track_beats_raw(pool, track_id).await?;
...
/// Tauri: get beat grid data for a track
#[tauri::command]
pub async fn get_track_beats(db: State<'_, Db>, track_id: i64) -> Result<Option<BeatGrid>, String> {
    get_track_beats_pool(&db.0, track_id).await
}

pub async fn delete_track_pool(
    pool: &SqlitePool,
    app_handle: AppHandle,
...
pub async fn delete_track(
    ...
    delete_track_pool(&db.0, app_handle, stem_cache, track_id).await
}

pub async fn wipe_tracks_pool(pool: &SqlitePool, app_handle: AppHandle) -> Result<(), String> {
...
pub async fn wipe_tracks(db: State<'_, Db>, app_handle: AppHandle) -> Result<(), String> {
    wipe_tracks_pool(&db.0, app_handle).await
}
...
// ============================================================================
// Database helper functions (used by other modules)
// ============================================================================

/// Get track file path and hash by ID
...
/// Get track stems (stem_name, file_path) for a track
...
/// Get track root analysis data (sections_json, logits_path)
...
/// Get beat grid data for a track (internal helper, returns raw DB data)
