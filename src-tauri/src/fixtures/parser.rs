...
pub fn build_index(root_path: &Path) -> Result<FixtureIndex, std::io::Error> {
    ...
    for entry in WalkDir::new(root_path).min_depth(2).max_depth(2).into_iter().filter_map(|e| e.ok()) {
        let path = entry.path();
        if path.extension().and_then(|s| s.to_str()) == Some("qxf") {
            ...
            if let Ok(relative) = path.strip_prefix(root_path) {
                ...
                if let Some(parent) = relative.parent() {
                     let manufacturer = parent.file_name().unwrap_or_default().to_string_lossy().to_string();
                     let model_filename = path.file_stem().unwrap_or_default().to_string_lossy().to_string();
