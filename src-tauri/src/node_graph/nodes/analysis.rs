    ...
    match node.type_id.as_str() {
        "harmony_analysis" => {
            let audio_edge = incoming_edges
                ...
                .ok_or_else(|| format!("HarmonyAnalysis node '{}' missing audio input", node.id))?;
        ...
        "mel_spec_viewer" => {
            if compute_visualizations {
                ...
                let beat_grid = incoming_edges
                    ...
                    .as_ref()
                    .map(|grid| {
                        crate::node_graph::context::beat_grid_relative_to_crop(
                            grid,
                            audio_buffer.crop.as_ref(),
                        )
                    });
