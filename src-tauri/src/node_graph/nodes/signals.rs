...
pub async fn run_node(
    ...
    match node.type_id.as_str() {
                ...
                for arg in ctx.arg_defs {
                    ...
                    match arg.arg_type {
                        PatternArgType::Color => {
                            ...
                            state.color_views.insert(format!("{}:{}", node.id, arg.id), color_json);
                        }
            ...
            "round" => {
                ...
                let Some(signal) = state.signal_outputs
            ...
            "ramp" => {
                ...
                let grid = grid_edge.and_then(|edge| {
                    state.beat_grids.get(&(edge.from_node.clone(), edge.from_port.clone()))
                });
            ...
            "ramp_between" => {
                ...
                let start_signal = start_edge.and_then(|edge| {
                    state.signal_outputs.get(&(edge.from_node.clone(), edge.from_port.clone()))
                });
            ...
            "remap" => {
                ...
                let Some(edge) = input_edge else { return Ok(true); };
                let Some(signal) =
                    state.signal_outputs.get(&(edge.from_node.clone(), edge.from_port.clone()))
                else {
                    eprintln!(
            ...
            "look_at_position" => {
                ...
                let Some(selection) =
                    state.selections.get(&(sel_e.from_node.clone(), sel_e.from_port.clone()))
                else {
                    return Ok(true);
                };
