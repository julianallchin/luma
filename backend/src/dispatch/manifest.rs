...
//! block — Host events have no registry to read. Everything else is
...
pub(super) fn check(table: &[Command]) -> Result<(), String> {
    ...
    let json = render_json(table, &root, &previous, &events);
    let markdown = render_markdown(table, &events);
...
/// Wire spelling of an argument: The dispatcher maps handler parameters
...
fn prose_by_command(previous: &Value) -> BTreeMap<String, Value> {
    ...
    for command in previous["commands"].as_array().into_iter().flatten() {
        ...
        let prose = command.get("prose").cloned().unwrap_or(Value::Null);
...
fn render_json(table: &[Command], root: &Path, previous: &Value, events: &[Value]) -> String {
    ...
    let manifest = json!({
        ...
        "version": 2,
        "source": "generated from the `commands!` table in backend/src/dispatch/mod.rs by `cargo test -p luma ipc_manifest`",
        "summary": {
...
fn render_markdown(table: &[Command], events: &[Value]) -> String {
    ...
    let _ = writeln!(
        out,
        "**{} commands** across **{} domains** · **{} events**\n",
        table.len(),
        domains.len(),
        events.len(),
    );
