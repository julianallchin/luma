...
/// Stable application identifier used by existing installations.
const APP_IDENTIFIER: &str = "com.luma.luma";

/// Root of Luma's durable on-disk data (the platform app config directory).
///
...
impl StorageRoot {
    /// Use an explicitly resolved application config directory.
    pub fn from_path(p: PathBuf) -> Self {
        Self(p)
    }

    /// Resolve the existing per-platform application config directory.
