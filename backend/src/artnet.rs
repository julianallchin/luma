...
impl ArtNetManager {
    pub async fn new(pool: &sqlx::SqlitePool, fixtures_root: PathBuf) -> Result<Self, String> {
        let settings = crate::settings::load_settings(pool).await?;
        let outputs = crate::database::local::outputs::list(pool).await?;
        let inner = Arc::new(Mutex::new(ArtNetInner {
            ...
            outputs: outputs
                .into_iter()
                .map(|output| (output.universe, output))
                .collect(),
