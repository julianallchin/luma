...
//! profile_score SCORE_ID OUTPUT [width height seconds] [all|no-haze|uniform]
use luma_lib::{
    eval::{Arena, Scope},
...
async fn main() -> Result<(), String> {
    ...
    if !["all", "no-haze", "uniform"].contains(&mode) {
    ...
    for i in 0..(seconds * 144 + 60) {
        ...
        let timing = renderer
            ...
            .map_err(|e| e.to_string())?;
        if i >= 60 {
            rows.push(serde_json::json!({"time":time,"eval_ms":eval_ms,"build_ms":build_ms,"gpu_ms":timing.gpu_total_ms,"scene_ms":timing.gpu_scene_ms,"haze_ms":timing.gpu_volumetric_ms,"grid_ms":timing.gpu_fog_grid_ms,"composite_ms":timing.gpu_composite_ms,"encode_ms":timing.cpu_encode_submit_ms,"cluster_ms":timing.cpu_cluster_ms,"cones":frame.fixture_cones.len(),"shadow_redraws":renderer.shadow_stats().redrawn_maps}));
        }
    }
