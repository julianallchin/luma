...
mod envelope_migration_tests {
    ...
    fn v2_softness_becomes_an_explicit_envelope_without_changing_clip_inputs() {
        // A frozen v2 fixture: serializing today's chase recipe can introduce
        // newer argument types before this test ever reaches the migration.
        let semantic = serde_json::json!({
            "schemaVersion": 2,
            "nodes": [
                {"id": "pattern_args", "typeId": "pattern_args", "params": {}},
                {"id": "effect", "typeId": "lighting/chase", "params": {}}
            ],
            "edges": [{
                "id": "input-softness", "fromNode": "pattern_args", "fromPort": "softness",
                "toNode": "effect", "toPort": "softness"
            }],
            "args": [{"id": "softness", "name": "Edge softness", "argType": "Proportion", "defaultValue": 0.3}]
        });
        let layout = serde_json::json!({"schemaVersion": 2, "nodes": {
            "pattern_args": {"positionX": 0.0, "positionY": 0.0},
            "effect": {"positionX": 320.0, "positionY": 0.0}
        }});
