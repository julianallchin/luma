//! Ported from zeron (MIT, © 2026 Wing) — crates/ui/src/markdown/mend.rs
//!
...
pub fn close_hanging(text: &str) -> Option<String> {
    ...
    if let Some((ticks, cpos)) = code.filter(|(_, cpos)| last_content.is_some_and(|lc| lc >= *cpos))
    {
    ...
    if let Some(&open) = brackets
        .last()
        .filter(|open| last_content.is_some_and(|lc| lc > **open))
    {
...
fn delim(
    ...
    let mut rest = run;
    if let Some(k) = can_close
        .then(|| delims.iter().rposition(|d| d.ch == c))
        .flatten()
    {
