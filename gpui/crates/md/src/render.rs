//! Ported from zeron (MIT, © 2026 Wing) — crates/ui/src/markdown/render.rs
//!
...
impl RenderCache {
    ...
    ///
    /// The cache is keyed by *position*, not by content, so an entry whose
    /// block was reparsed is stale and nothing in this module can tell. Only
    /// the parser knows, and it says so with
    /// [`crate::parser::IncrementalParser::stable_prefix_blocks`]: everything
    /// below that boundary is byte-identical and everything from it may have
    /// changed. A caller that appends text and does not call this paints the
    /// text as it stood when the block was first flattened.
    pub fn invalidate_from(&mut self, row: &str, from: usize) {
        self.flats
            .retain(|(r, top, _), _| r.as_ref() != row || *top < from);
        self.code
            .retain(|(r, top, _), _| r.as_ref() != row || *top < from);
    }
...
/// One fenced block's tokens, or `None` when it paints plain.
pub type CodeHighlight<'a> = Option<&'a HighlightedCode>;

/// Render a whole tree stacked with the md block gap.
///
/// `highlighter` is asked only about fenced blocks; [`crate::NoHighlight`]
/// paints them plain, and the layout is identical either way.
pub fn render_tree(
    tree: &BlockTree,
    opts: &RenderOptions,
    theme: &Theme,
    ...
    div()
        ...
        .children(tree.blocks.iter().enumerate().map(|(ix, top)| {
            let document = match &top.block {
                Block::CodeBlock { language, code } => {
                    highlighter.highlight(language.as_deref(), code)
                }
                _ => None,
            };
            render_block(&top.block, ix, ix, opts, theme, window, document.as_deref())
        }))
        .into_any_element()
}
...
/// and inline-code ranges.
///
/// The code ranges' rounded washes are painted by a canvas *under* the text:
/// `TextRun::background_color` can only paint square boxes. `text` is a
/// `SharedString` so cached reuse across frames is an `Arc` clone.
pub struct FlatText {
...
pub(crate) fn range_rects(
    ...
    while cur < range.end && guard < 256 {
        ...
        match layout.position_for_index(seg_end) {
            Some(p2) if p2.x > p1.x => rects.push(Bounds::new(
...
fn render_code_block(
    ...
    let hl_key = highlight.map_or((0, 0), |h| (h.lines.as_ptr() as usize, h.lines.len()));
    let build = || {
...
///
/// The font is the same on every run, so recoloring can never change layout —
/// which is what lets a highlighter arrive late, or not at all.
pub fn runs_for_syntax_line(
    ...
    runs_for_syntax_line_with_plain(line, spans, mono, theme.code_text)
}

/// [`runs_for_syntax_line`] over an explicit untokenized color.
...
mod tests {
    use super::*;
    use crate::parser::InlineStyle;

    /// A token span recolors exactly its own bytes and nothing else — the
    /// property that makes highlighting pure paint.
    #[test]
