        ...
        div()
            ...
            .child(
                div()
                    ...
                    .child(sample(
                        "Shell blur stripes",
                        Some(luma_ui::dialog::SHELL_BLUR),
                    ))
                    .child(sample(
                        "Sidebar blur stripes",
                        Some(luma_ui::dialog::SIDEBAR_BLUR),
                    )),
            )
            .child(
                div()
...
#[test]
fn shell_and_sidebar_reduce_real_pixel_edges_at_distinct_strengths() {
    ...
    let shots = run(
        ...
        ({
            ...
            shell: capture("Shell blur stripes"),
            sidebar: capture("Sidebar blur stripes"),
    ...
    preserve(shots["shell"].as_str().unwrap(), "dialog-blur-shell.png");
    preserve(
        shots["sidebar"].as_str().unwrap(),
        "dialog-blur-sidebar.png",
    );
    preserve(
    ...
    let shell = edge_energy(&pixels(shots["shell"].as_str().unwrap()));
    let sidebar = edge_energy(&pixels(shots["sidebar"].as_str().unwrap()));
    ...
    println!("dialog blur edge energy: plain={plain:.3}, shell={shell:.3}, sidebar={sidebar:.3}");

    assert!(
        shell < plain * 0.6,
        "shell blur did not reduce edges: {plain} -> {shell}"
    );
    assert!(
        sidebar < shell * 0.9,
        "stronger sidebar blur did not reduce edges further: {shell} -> {sidebar}"
    );
    assert!(
