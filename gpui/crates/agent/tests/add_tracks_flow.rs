...
const SCRIPT: &str = r#"
    ...
    const initial = until("the all-Luma browser", (s) =>
        s.find({ role: "input", label: "Search all tracks…" }) !== undefined
            && s.find({ role: "row", label: "Aurora" }) !== undefined
            && s.find({ role: "button", label: "Import tracks" }) !== undefined);
    const auroraRow = initial.findAll({ role: "row", label: "Aurora" })
    ...
    const retry = until("Needle in the retry browser", (s) =>
        s.findAll({ role: "row", label: "Needle" })
            .some((row) => row.bounds.width > 300));
    app.click(retry.findAll({ role: "row", label: "Needle" })
        .find((row) => row.bounds.width > 300));
