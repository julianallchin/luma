...
fn harness(window: (f32, f32), views: HashMap<String, Signal>) -> Harness {
...
fn the_gauntlet_patterns_are_captured_from_the_native_editor() {
    ...
    for shot in SHOTS {
        ...
        let mut harness = harness(shot.window, views);
        let result = harness.exec(
            &script(shot.pattern, shot.anchor.map(|(title, _, _)| title)),
