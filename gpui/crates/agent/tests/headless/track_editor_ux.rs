...
//!
//! [`support::Fixture`] seeds a library per test, so the reason is not
//! isolation — it is that these clusters share one opening geometry, and
//! re-reaching it per cluster would be the same walk paid five times on a
//! suite already bound by the wall clock. So the
