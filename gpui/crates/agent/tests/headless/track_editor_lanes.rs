...
//!
//! A separate test from `track_editor_ux` because this contract needs a score
//! the other one's assertions could not survive. Not a separate *process*:
//! each fixture carries its own library directory on the pump thread's
//! [`luma_ui::runtime::Runtime`], so the two coexist in one binary.
...
const SCRIPT: &str = r#"
    ...
    until("the timeline", (s) => s.find({ role: "card", label: "Waveform" }) !== undefined);
        nav.expand();
    // This is a test about the lane stack's own geometry, so give the editor
    // the whole column: the stage over it would otherwise take 40% of the
    // height and clip the lanes this walk has to reach.
    nav.stageOff();
