...
const SCRIPT: &str = r#"
    ...
    const entry = {
        // The picker's header is a search field now, not a title, so there is
        // no "Choose a venue" prose to look for. The dialog card itself is the
        // honest tripwire for "the app entered on the venue picker" — and it
        // is the node venues.rs and dialog_focus.rs already assert on.
        chooser: home.find({ role: "card", label: "Venue dialog" }) !== undefined,
