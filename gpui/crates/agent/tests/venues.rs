...
fn venue_launch_picker_create_and_stale_reads_are_correlated() {
    ...
    let out = exec(
        ...
        const opened = until("the created venue", (s) =>
...
#[test]
fn switching_venues_revokes_the_track_subject_in_the_new_tab_menu() {
    let dir = fixture_dir(
        "tab-subject",
        &[("alpha", "Alpha Hall"), ("beta", "Beta Room")],
        None,
    ...
    let out = exec(
        ...
        const track = menu.find({ role: "button", label: "Track editor" });
        ({
            enabled: track.enabled,
            reason: menu.find({ role: "text", label: "Select a track first" }) !== undefined,
            oldTabKept: menu.find({ role: "button", label: "Alpha Hall Track" }) !== undefined,
        })
        "#,
    );
    assert_eq!(
    ...
    assert_eq!(
        out["oldTabKept"], true,
        "venue switch destroyed an open track tab: {out:#}"
    );
}
