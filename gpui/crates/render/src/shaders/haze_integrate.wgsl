...
fn integrate_grid(@builtin(workgroup_id) column: vec3<u32>,
    ...
    for (var tap = 0u; tap < 4u; tap += 1u) {
        ...
        density_integral += haze_noise(haze.camera_pos.xyz + ray.dir * t, haze.params.w)
            * exp(-haze.depth.z * t);
    }
    segments[z] = vec4<f32>(lighting * (haze.depth.z * density_integral * (end - start) * 0.25), 0.0);
    workgroupBarrier();
    for (var stride = 1u; stride < FOG_SLICES; stride *= 2u) {
        var prior = vec4<f32>(0.0);
