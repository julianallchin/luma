...
//! # What v1 is
//!
//! Open, look, listen, select, and slide clips around in time. Selecting one
//! or many clips, sweeping a range out of empty lane, moving clips sideways
//! and pulling their edges all work and persist through `update_track_score`.
//!
//! What does not exist yet needs a seam that is not there: creating and
//! deleting clips, a z-index write (so a drag between lanes), an undo stack,
//! a clipboard, and a loop region. Vertical zoom is fixed at 1 and there is no
//! vertical scroll, which is also why the lanes are top-anchored here and
//! bottom-anchored on the web.
//!
...
//! 112 .. 192    row 0        the empty insertion lane; clears too
//! 192 ..        lanes        clip headers grab; everything else sweeps
//! ```
//!
...
pub struct Editor {
    ...
    /// The edits waiting to be written. A group drag moves every selected
    /// clip, so this is a queue drained one write at a time; a second drag of
    /// the same clip supersedes the first rather than queueing behind it.
    pending: Vec<Edit>,
...
impl Editor {
    ...
    fn clip_at(&self, time: f64, row: usize, y: f32) -> Option<&Clip> {
        let header = TRACK_START_Y + row as f32 * LANE_HEIGHT + 1. + CLIP_HEADER;
        if y >= header {
            return None;
        }
        self.clips
    ...
    fn drag_clips(&mut self, gesture: &Gesture, delta: f64) {
        let Gesture::Clips {
            pressed,
            drag,
            initial,
        ...
        let clips: Vec<Clip> = self
            ...
            .map(|clip| {
                ...
                if let Some(was) = held.get(clip.id.as_ref()) {
                    match drag {
                        Drag::Move => {
                            start = (was.start + shift).max(0.);
                            end = start + (was.end - was.start);
                        }
                        Drag::Resize(Edge::Start) => {
    ...
    /// Queue every dragged clip's current bounds as a write, replacing any
    /// earlier write still waiting for the same clip.
    fn queue_edits(&mut self, ids: &[SharedString]) {
        for id in ids {
            ...
            match self
                .pending
                .iter_mut()
                .find(|queued| queued.clip == edit.clip)
            {
                Some(queued) => *queued = edit,
...
impl Luma {
    ...
    fn timeline_press(&mut self, at: Point<Pixels>, shift: bool, cx: &mut Context<Self>) {
        let mut seek = None;
        self.with_track_editor(cx, |editor| {
            ...
            let rows = lane_count(&editor.clips);
            let Some(row) = row_at(y).filter(|row| *row < rows) else {
                editor.deselect();
                return;
            };
            ...
            let initial: Rc<[Initial]> = if already {
                editor
                    .clips
                    .iter()
        ...
        self.with_track_editor(cx, |editor| {
            ...
            match &gesture {
                ...
                &Gesture::Marquee { row, start } => {
                    ...
                    let rows = lane_count(&editor.clips);
                    let current = row_at(y).unwrap_or(0).min(rows.saturating_sub(1));
                    let cursor = Cursor {
                        row,
                ...
                &Gesture::Clips { origin, .. } => {
                    let delta = f64::from(f32::from(at.x - origin) / editor.view.zoom);
                    editor.drag_clips(&gesture, delta);
                    editor.sync_cursor();
                }
            }
            ...
            match editor.gesture.take() {
                Some(Gesture::Clips {
                    ...
                    let ids: Vec<SharedString> =
                        initial.iter().map(|clip| clip.id.clone()).collect();
                    editor.queue_edits(&ids);
                    save = !editor.pending.is_empty();
                }
    ...
    ///
    /// `rate` is `None` for a bare wheel, which scrolls rather than zooms —
    /// the web's bare wheel is the scroll container's and this canvas has to
    /// stand in for one. Otherwise it is the exponential rate the modifier
    /// asks for, and the zoom is anchored on a *latched* point: whatever was
    ...
    fn timeline_wheel(
        ...
        self.with_track_editor(cx, |editor| {
            ...
            let Some(rate) = rate else {
                editor.anchor = None;
                let scroll = editor.view.scroll - delta.x - delta.y;
    ...
    fn save_clip(&mut self, cx: &mut Context<Self>) {
        ...
        // Oldest first: the queue is a group drag's clips in the order they
        // were captured, and a later edit to the same clip already replaced
        // the earlier one in place.
        let edit = state.pending.remove(0);
        state.saving = true;
        state.error = None;
        ...
        cx.spawn(async move |this, cx| {
            ...
            this.update(cx, |this, cx| {
                ...
                this.with_track_editor(cx, |editor| {
                    ...
                    match result {
                        Ok(saved) => {
                            if editor.pending.is_empty() && editor.gesture.is_none() {
                                let labels = editor
                                    .clips
                                    .iter()
...
fn toolbar(state: &Editor, app: &Entity<Luma>) -> Div {
    ...
    div()
        ...
        .when(
            state.writable() && (state.saving || !state.pending.is_empty()),
...
fn register(scene: &Scene, canvas: Bounds<Pixels>, window: &Window, cx: &mut App) {
    ...
    // Each lane as a row, so the empty space between clips is addressable.
    // A press there is a real gesture — it sweeps a range — and a surface a
    // script cannot name is a gesture it cannot make.
    for lane in 0..lane_count(&scene.clips) {
        agent_paint_node(
            Role::Row,
            format!("Lane {lane}"),
            Bounds {
                origin: point(
                    canvas.origin.x,
                    canvas.origin.y + px(TRACK_START_Y + lane as f32 * LANE_HEIGHT),
                ),
                size: size(canvas.size.width, px(LANE_HEIGHT)),
            },
            window,
            cx,
        );
    ...
    for clip in scene.clips.iter() {
        ...
        // The *grabbable* extent, not the drawn one: only a clip's header bar
        // answers the pointer, so a node covering its whole lane would send
        // every scripted click into the inert body.
        let header = Bounds {
            origin: box_.origin,
    ...
    // The cursor is a control in the sense that matters here: it is where the
    // next edit lands, and nothing else on the canvas reports it.
        ...
        agent_paint_node(
            ...
            Bounds {
                origin: point(
                    ...
                    canvas.origin.y + px(TRACK_START_Y + min_row as f32 * LANE_HEIGHT),
                ),
                size: size(
                    px((scene.view.x_of(to) - left).max(2.)),
                    px((max_row - min_row + 1) as f32 * LANE_HEIGHT),
                ),
            },
            window,
            cx,
...
fn listen(app: &Entity<Luma>, hitbox: &Hitbox, window: &mut Window) {
    ...
    window.on_mouse_event(move |event: &MouseDownEvent, phase, window, cx| {
        ...
        pressed.update(cx, |this, cx| this.timeline_press(at, shift, cx));
    });
    ...
    window.on_mouse_event(move |event: &ScrollWheelEvent, phase, window, cx| {
        ...
        let rate = if event.modifiers.control {
            Some(View::ZOOM_PER_PIXEL_PINCH)
        } else if event.modifiers.secondary() {
            Some(View::ZOOM_PER_PIXEL)
        } else {
            None
        };
        let delta = point(f32::from(wheel.x), f32::from(wheel.y));
        let at = event.position;
        zoomed.update(cx, |this, cx| this.timeline_wheel(at, delta, rate, cx));
    });
}
...
fn paint(bounds: Bounds<Pixels>, scene: &Scene, window: &mut Window, cx: &mut App) {
    ...
    window.with_content_mask(Some(ContentMask { bounds }), |window| {
        ...
        if let Some(cursor) = scene.cursor {
            paint_cursor(bounds, cursor, scene.view, start, end, window);
        }
...
fn paint_lanes(canvas: Bounds<Pixels>, scene: &Scene, window: &mut Window) {
    ...
    let lanes = lane_count(&scene.clips);
    for lane in 0..lanes {
...
fn paint_cursor(
    ...
    let (min_row, max_row) = cursor.rows();
    let top = TRACK_START_Y + min_row as f32 * LANE_HEIGHT;
    let height = (max_row - min_row + 1) as f32 * LANE_HEIGHT;
