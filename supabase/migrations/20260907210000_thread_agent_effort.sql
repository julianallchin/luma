ALTER TABLE public.agent_threads ADD COLUMN effort text;
...
AS $$
BEGIN
    IF (to_jsonb(NEW) - ARRAY['sync_seq', 'title', 'engine', 'model', 'provider', 'effort', 'lifecycle_state', 'updated_at'])
        IS DISTINCT FROM (to_jsonb(OLD) - ARRAY['sync_seq', 'title', 'engine', 'model', 'provider', 'effort', 'lifecycle_state', 'updated_at'])
    THEN
