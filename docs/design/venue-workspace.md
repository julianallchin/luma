...
- One Venue tab: live stage above, with the lower panel divided vertically into fixtures on the left and groups on the right. The two columns scroll independently and keep their headers visible. There are no Stage / Fixtures / Groups navigation tabs.
...
- Selecting a fixture opens its label, mode, universe, address, authored group membership, and geometry controls above the left list. The inspector scrolls with the fixtures so it cannot squeeze the inventory out of view. Unplaced fixtures offer Place. Add uses the existing fixture picker and allocator. Viewport selection and inventory selection stay in sync; Shift or the platform modifier extends selection.
...
Column layout validation: all 10 patch/group/render-settings tests pass, including group creation, assignment, rename, reparenting and cancellation at 1100 × 900. The workspace/all-targets check and native build pass. Native X11 captures were inspected using the actual EBF library with chat visible, including both lists, fixture selection and group editing. No venue contents were changed for captures. The preceding revision also passed all 15 builder tests and the venue-filter browser test.
