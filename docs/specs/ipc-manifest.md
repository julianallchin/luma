...
Canonical contract for every Tauri command and event crossing the Rust/TS boundary.
Machine-readable source of truth: [`ipc-manifest.json`](./ipc-manifest.json) — this file is
the human overview of it. Both come from a one-time audit of every command, cross-checked against a
`#[tauri::command]` signature scan of `src-tauri/src` (arg names, injected state, and async-ness were
taken from the code, not the audit, wherever the two disagreed). **This is a snapshot, not a live
...
| Domain | Commands | Rust module | Dead |
| --- | ---: | --- | ---: |
| `agent` | 10 | `commands/agent_threads.rs (+2)` |  |
| `agent/authored-state` | 11 | `commands/authored_state.rs` |  |
| `agent/patterns` | 1 | `commands/authored_state.rs` |  |
| `agent/preview` | 3 | `annotation_preview.rs` |  |
| `artnet` | 3 | `artnet.rs` |  |
| `audio-host` | 10 | `host_audio.rs` | 1 |
| `auth` | 5 | `commands/auth.rs` | 1 |
| `auth/profiles` | 1 | `commands/cloud_sync.rs` |  |
| `dj-import` | 5 | `commands/rekordbox.rs` |  |
| `dj-import/engine-dj` | 8 | `commands/engine_dj.rs` | 1 |
| `dj-import/tracks` | 1 | `commands/rekordbox.rs` |  |
| `node-graph` | 1 | `commands/node_graph.rs` |  |
| `node-graph/eval` | 1 | `commands/node_graph.rs` |  |
| `node-graph/patterns` | 1 | `commands/node_graph.rs` |  |
| `patterns` | 11 | `commands/patterns.rs (+2)` | 1 |
| `patterns/authored-state` | 3 | `commands/patterns.rs` |  |
| `perform` | 12 | `commands/perform.rs (+1)` |  |
| `perform/midi` | 16 | `commands/midi.rs` | 1 |
| `perform/midi-controller` | 9 | `commands/controller.rs` | 1 |
| `perform/midi-mixer` | 8 | `commands/mixer.rs` |  |
| `score-dsl` | 2 | `commands/score_dsl.rs` |  |
| `score-dsl/authored-state` | 1 | `commands/score_dsl.rs` |  |
| `scores` | 7 | `commands/scores.rs` |  |
| `settings` | 2 | `settings.rs` |  |
| `stage` | 5 | `commands/stage.rs` |  |
| `sync` | 7 | `commands/sync.rs` | 5 |
| `telemetry` | 1 | `commands/telemetry.rs` |  |
| `track-editor/scores` | 1 | `commands/scores.rs` |  |
| `tracks` | 12 | `commands/tracks.rs` | 1 |
| `tracks/audio` | 1 | `commands/tracks.rs` | 1 |
...
| `venues` | 8 | `commands/venues.rs` |  |
| **total** | **196** | | **15** |

## Events

Tauri events are the only push channel; there is no subscription command. Every long-running
command reports progress here, not in its return value.

| Event | Emitters | Listeners | Note |
| --- | ---: | ---: | --- |
| `close-requested` | 0 | 1 | emitted by Tauri itself (window close), not by our code |
| `controller_port_change` | 1 | 0 | **orphan** — emitted, nobody listens |
| `controller_state` | 3 | 1 |  |
| `dmx://update` | 0 | 1 | **dead listener** — nothing emits this anywhere |
| `engine-dj-import-progress` | 2 | 2 |  |
| `file-import-progress` | 3 | 1 |  |
| `host-audio://state` | 1 | 2 |  |
| `library-changed` | 5 | 2 |  |
| `midi_learn_captured` | 1 | 3 |  |
| `mixer_learned` | 1 | 1 |  |
| `mixer_state` | 2 | 1 |  |
| `open-settings` | 1 | 1 |  |
| `perform_event` | 2 | 1 |  |
...
| `track-import-progress` | 3 | 2 |  |
| `track-status-changed` | 1 | 1 |  |
| `universe-buffer` | 0 | 1 | **dead listener** — nothing emits this anywhere |
| `universe-state-update` | 2 | 1 |  |
