...
export function FrequencyAmplitudeNode(props: NodeProps<BaseNodeData>) {
	...
	const [selectedBinIds, setSelectedBinIds] = React.useState<string[]>(() => {
		try {
			const storedRangesJson =
				(params.selected_frequency_ranges as string) ?? "[]";
	...
	React.useEffect(() => {
		const selectedRanges = FREQ_BINS.filter((bin) =>
	...
	const overallMinFreq = React.useMemo(() => {
		...
		return Math.min(
			...FREQ_BINS.filter((bin) => selectedBinIds.includes(bin.id)).map(
				(bin) => bin.min,
			),
		);
	}, [selectedBinIds]);
