...
type TrackEditorState = {
	...
	createAnnotation: (input: Omit<CreateAnnotationInput, "trackId">) => Promise<TrackAnnotation | null>;
	updateAnnotation: (input: UpdateAnnotationInput) => Promise<TrackAnnotation | null>;
...
export const useTrackEditorStore = create<TrackEditorState>((set, get) => ({
	...
	loadTrack: async (trackId: number, trackName: string) => {
		...
		try {
			const beatGrid = await invoke<BeatGrid | null>("get_track_beats", { trackId });
		...
		try {
			const waveform = await invoke<TrackWaveform>("get_track_waveform", { trackId });
			set({ waveform, waveformLoading: false, durationSeconds: waveform.durationSeconds });
		} catch (err) {
		...
		try {
			const rawAnnotations = await invoke<TrackAnnotation[]>("list_annotations", { trackId });
	...
	syncPlaybackState: (snapshot: PlaybackStateSnapshot) => {
		...
		} else if (snapshot.isPlaying && snapshot.activeNodeId !== `track:${trackId}`) {
			set({ isPlaying: false });
		}
	},
	...
	selectAnnotation: (annotationId: number | null) => set({ selectedAnnotationId: annotationId }),
	setDraggingPatternId: (patternId: number | null) => set({ draggingPatternId: patternId }),
	...
	updateAnnotation: async (input) => {
		...
		try {
			const updated = await invoke<TrackAnnotation>("update_annotation", { input });
			...
			set({ annotations: annotations.map((a) => (a.id === updated.id ? enriched : a)) });
			return updated;
		} catch (err) {
