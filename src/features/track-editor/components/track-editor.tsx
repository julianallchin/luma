...
function Timecode() {
	...
	// Calculate beat and bar from playhead position
	const getTimecode = () => {
		...
		const totalBeats =
			(playheadPosition - beatGrid.downbeatOffset) / beatLength;
		const barNumber = Math.floor(totalBeats / beatGrid.beatsPerBar) + 1;
		const beatInBar = Math.floor(totalBeats % beatGrid.beatsPerBar) + 1;
		return {
			bar: `${barNumber}.${beatInBar}`,
			beat: Math.floor(totalBeats),
		};
	};
