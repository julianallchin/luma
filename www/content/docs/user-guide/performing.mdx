...
Luma includes a real-time 3D visualizer that renders your light show as it plays. The visualizer shows your venue's fixture layout with accurate beam positions, colors, and effects.

### Render Settings

A render settings popover lets you adjust the visual quality and effects:

| Setting | Description |
|---------|-------------|
| **Dark Stage** | Dims the stage environment so light beams stand out more dramatically |
| **Volumetric Haze** | Enables raymarched atmospheric haze with Mie scattering, making light beams visible in the air (like a real haze machine) |
| **Haze Steps** | Controls the raymarching quality -- higher values produce smoother haze at the cost of performance |
| **Haze Density** | How thick the atmospheric haze appears |
| **Bloom** | Adds a glow effect around bright light sources |

The volumetric haze system simulates realistic light scattering through fog/haze, giving you a preview that closely matches what the show looks like in a real venue with haze.
...
Within each pattern, Apply node outputs are merged. Multiple Apply nodes in a single pattern (e.g., one for color and one for strobe) merge their outputs into a single layer. If two Apply nodes try to write the same capability to the same fixture, that is a conflict -- keep your patterns clean with one Apply per capability per fixture group.
...
The composited values (abstract concepts like "color," "brightness," "pan," "tilt") are converted to specific DMX channel values based on each fixture's definition. Luma looks up which DMX channels correspond to which capabilities using the fixture definition's channel map.
