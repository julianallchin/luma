...
Venues can be shared with other users for collaborative light show creation. When you share a venue, other users can **join** it and see all the fixtures and groups you have set up.

### Owner vs. Member

| | Owner | Member |
|---|---|---|
| **Edit fixtures** | Yes | No (read-only) |
| **Edit groups** | Yes | No (read-only) |
| **Create scores** | Yes | Yes |
| **View scores** | Yes | Yes |
| **Venue settings** | Yes (rename, delete) | No |

Venue members see the universe designer in **read-only mode** -- they can view the fixture layout and groups but cannot modify them. This ensures the venue owner controls the physical setup while collaborators focus on creating their own light shows (scores) for the shared fixture layout.

### Ungrouped Fixtures

Before a venue is ready for collaboration, all fixtures must be assigned to at least one group. If any fixtures are ungrouped, Luma shows a warning banner and blocks navigation away from the universe tab until all fixtures are grouped. This ensures patterns always have meaningful group names to target.

## Next Steps

Once your fixtures are patched and positioned, the next step is to [create groups](/docs/user-guide/groups-and-tags) -- this is what makes your shows portable.
