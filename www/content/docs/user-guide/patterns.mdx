...
### 4. Add an Apply Color node

Connect the Select output to Apply Color's "Selection" input, and the Gradient output to Apply Color's "Signal" input.

That is the complete graph. Four nodes (plus the two auto-added inputs), three wires. The result: every fixture in the venue flashes to full brightness on each beat and fades to black by the next beat. Apply Color automatically derives brightness from the color -- white means full on, black means off.
...
<NodeGraph
  ...
  nodes={[
    ...
    { id: "select", label: "Select", category: "selection", x: 240, y: 260, params: { tag: "all" }, outputs: ["Selection"] },
    { id: "apply", label: "Apply Color", category: "output", x: 500, y: 80, inputs: ["Signal", "Selection"] },
  ]}
  edges={[
...
7. **Gradient** -- Maps the narrow pulse to a color (black at 0, white at 1).
8. **Apply Color** -- Connect the Select output and the Gradient output.
...
2. **Select** targets moving head fixtures.
