...
## From Rekordbox or Engine DJ

Luma integrates directly with **Rekordbox** and **Engine DJ** music libraries. When you connect to your library:
...
The Engine DJ workflow is recommended if you use Denon DJ gear, since Luma's live performance mode also connects to Denon hardware via StageLinQ.
...
The audio analysis pipeline runs on a Python environment that Luma manages automatically. On first launch, Luma sets up a Python 3.12 virtual environment in the background -- you will see toast notifications showing the progress. This happens without blocking the app, so you can start setting up your venue while it installs.

If your system has an NVIDIA GPU, Luma automatically detects the CUDA version and installs GPU-accelerated PyTorch for faster analysis. ffmpeg is bundled with the app, so no system installation is needed.
