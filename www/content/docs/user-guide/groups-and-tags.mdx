...
| Tag | Meaning |
|-----|---------|
| `hit` | High-intensity impact lights (strobes, blinders) |
| `wash` | General wash/fill lighting |
| `accent` | Focused accent/spot lighting |
| `chase` | Fixtures used for sequential chase effects |

You assign tags to groups when setting up a venue. A group can have multiple tags -- "Front Wash" might be tagged `wash` and `low`.

The special token `all` matches every fixture in the venue.

## Why Tags Are the Magic

When you build a pattern ([Step 4](/docs/user-guide/patterns)), you do not say "animate fixtures 1 through 8." You say "animate everything tagged `circular`" or "strobe everything tagged `left & wash`."
...
|----------|------|---------|---------|
| `&` | AND | `left & wash` | Left-side wash fixtures |
| `\|` | OR | `left \| right` | Both sides of the room |
| `~` | NOT | `circular & ~hit` | Circular fixtures that are not hits |
| `>` | FALLBACK | `hit > accent` | Prefer hits; if none exist, use accents instead |

Parentheses work for grouping: `(left | right) & wash` means "left or right fixtures, but only if they are washes."
