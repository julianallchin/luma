...
- `audio_lowpass` -- Lowpass audio source for spectrum or energy analysis.
- `audio_highpass` -- Highpass audio source for spectrum or energy analysis.
- `envelope` -- Samples an editable curve at the input progress.
...
Extract spatial attributes from fixture arrangements. Fixture targeting is handled by the pattern's **Selection argument** (a group expression set when placing the pattern on a timeline), not by a node in the graph.
...
- `sample_gradient` -- Sample a gradient with a numerical position.
- `mix_palette` -- Mix palette colors using channel weights; expose opacity separately.
- `palette_fallback` -- Use another palette when the first has no colors.
- `hsv` -- Construct RGB from hue, saturation and value.
- `rotate_hue` -- Rotate RGB hue with fixture/time broadcasting.
- `core/channel_argmax` -- Find the strongest channel; compose pitch-driven hue rotation.
...
- `apply_color` -- Sets fixture color and brightness.
