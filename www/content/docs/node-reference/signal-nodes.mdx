...
- There is no `abs`: the node is binary, and `abs_diff` with `b` left unwired (default `0`) is `|a|`.
...
| **Type ID** | `sine_wave` |
| **Category** | Generator |
| **Description** | Beat-synced sinusoidal oscillator. |
...
| Port | Type |
|------|------|
| `out` | Signal (N=1, T=sim, C=1) |

### Parameters

| Name | Type | Default | Description |
|------|------|---------|-------------|
| `subdivision` | float | 1.0 | Cycles per beat (1 = one full cycle per beat, 0.5 = every 2 beats, 2 = twice per beat) |
...
- Derives frequency from the beat grid: `freq_hz = subdivision * (bpm / 60)`.
- Formula: `output = offset + amplitude * sin(2 * pi * freq_hz * t + phase_rad)`.
- Output range (with defaults): [-1.0, 1.0].
- Useful for beat-synced LFO effects (e.g., breathing at 0.25 = every 4 beats, color cycling at 0.5 = every 2 beats).
