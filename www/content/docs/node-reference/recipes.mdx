...
Draw an Envelope curve for brightness, then trigger a Pulse on each beat.

<NodeGraph
  nodes={[
    { id: "trigger", label: "Beat Trigger", category: "input", x: 0, y: 0, outputs: ["Events"] },
    { id: "pulse", label: "Pulse", category: "generator", x: 200, y: 0, inputs: ["Trigger"], outputs: ["Signal"] },
    { id: "output", label: "Output", category: "output", x: 400, y: 0, inputs: ["Dimmer"] },
  ]}
  edges={[
    { from: "trigger", fromHandle: "Events", to: "pulse", toHandle: "Trigger" },
    { from: "pulse", fromHandle: "Signal", to: "output", toHandle: "Dimmer" },
  ]}
/>

Set the trigger's Repeat interval and the Pulse's travel duration independently.
Draw the brightness curve in the inspector. Longer durations overlap with Max;
shorter durations leave a dark gap before the next trigger.
...
<NodeGraph
  height={420}
  nodes={[
    { id: "attr", label: "Get Attribute", category: "selection", x: 0, y: 0, inputs: ["Selection"], params: { attribute: "norm_index" }, outputs: ["Signal"] },
    ...
    { id: "apply", label: "Apply Color", category: "output", x: 800, y: 150, inputs: ["Signal"] },
  ]}
  edges={[
...
1. `get_attribute(normalized_index)` reads the pattern's fixture selection and assigns each fixture a value from 0.0 to 1.0 based on its position in the selection order.
2. `beat_clock` feeds `ramp`, which produces a monotonically increasing beat count (0, 1, 2, 3...).
3. `math(subtract)` subtracts the ramp from each fixture's index. This creates a value that decreases over time per fixture -- effectively a traveling wave.
4. `modulo(1.0)` wraps the result into the [0, 1) range, making the chase repeat every beat.
5. `falloff` shapes the transition. A narrow width creates a tight moving dot; a wide width creates a broad sweep.
6. `gradient` maps the shaped scalar to a color.
7. `apply_color` writes the result to the selected fixtures.
...
<NodeGraph
  nodes={[
    ...
    { id: "apply", label: "Apply Color", category: "output", x: 800, y: 40, inputs: ["Signal"] },
  ]}
  edges={[
...
6. `apply_color` writes the color to the fixtures targeted by the pattern's Selection argument. Since `Apply Color` derives brightness from the color, white = full on, black = off.
...
<NodeGraph
  height={420}
  nodes={[
    { id: "attr", label: "Get Attribute", category: "selection", x: 0, y: 0, inputs: ["Selection"], params: { attribute: "angular_pos" }, outputs: ["Signal"] },
    { id: "clock", label: "Beat Clock", category: "input", x: 0, y: 150, outputs: ["Beat Grid"] },
    { id: "ramp", label: "Time Ramp", category: "generator", x: 200, y: 150, inputs: ["Beat Grid"], outputs: ["Signal"] },
    { id: "math", label: "Math", category: "transform", x: 400, y: 50, inputs: ["A", "B"], params: { op: "subtract" }, outputs: ["Signal"] },
    { id: "mod", label: "Modulo", category: "transform", x: 400, y: 200, inputs: ["Signal"], params: { divisor: "1.0" }, outputs: ["Signal"] },
...
1. `get_attribute(angular_position)` reads the pattern's fixture selection (e.g., a `floor_ring` group) and uses PCA + RANSAC circle fitting to assign each fixture an angular position from 0.0 to 1.0 around the fitted circle. This works even for 3D arrangements.
2. `beat_clock` feeds `ramp`, producing a monotonically increasing beat count.
3. `math(subtract)` subtracts the ramp from each fixture's angular position, creating a rotating wave.
4. `modulo(1.0)` wraps the result so the chase repeats every beat.
5. `falloff(width=0.3)` shapes the beam width. At 0.3, roughly 30% of the circle is illuminated at any moment.
6. `gradient` maps the shaped scalar to a color (black at 0, white at 1).
7. `apply_color` writes the color to the selected fixtures. Brightness is derived from the color automatically.
...
<NodeGraph
  nodes={[
    ...
    { id: "apply", label: "Apply Color", category: "output", x: 640, y: 30, inputs: ["Signal"] },
  ]}
  edges={[
...
4. `apply_color` writes the harmonically-derived color to the fixtures targeted by the pattern's Selection argument.

### Variations

- Add `spectral_shift` after `chroma_palette` to apply additional hue rotation based on the dominant pitch.
- Use `harmonic_tension` with a `gradient` (dark→bright) feeding `apply_color` to make the lighting intensity increase during dissonant passages.
