...
File: `backend/src/services/groups.rs`
...
The `get_attribute` node (in `backend/src/node_graph/nodes/selection.rs`) extracts per-fixture scalar values from a Selection and outputs them as a Signal with `N = fixture count`, `T = 1`, `C = 1`.

| Attribute | Description |
|-----------|-------------|
| `index` | Integer order within the selection (0, 1, 2, ...) |
| `normalized_index` | Order normalized to 0.0-1.0 range |
| `count` | Number of items in the selection, on every item |
...
| `u` | Position along the rig's own horizontal axis (0.0-1.0) -- see [Rig-Intrinsic UV Space](#rig-intrinsic-uv-space) |
| `v` | Height within the selection (0.0-1.0), constant 0.5 when the rig is flat |
...
### Rig-Intrinsic UV Space

`rel_x` / `rel_y` are world axes: a "chase along X" authored in one venue becomes a diagonal in the next one if that rig is hung at an angle. `u` and `v` are the rig's *own* axes, so the same pattern reads the same way anywhere.

- **`u`** -- position along the first principal component of the selection's **horizontal** (XY) spread, normalized 0.0-1.0. For a straight bar of fixtures it is the bar; for an L or an arc it is the dominant run. It is sign-canonicalized so its largest world component is positive (broadly stage-right, or front-to-back for a rig that runs that way) -- a chase along `u` never silently reverses between venues. An axis has no inherent direction, so the direction is borrowed from the world: `u` survives any rig rotation within a 180 degree window and does reverse for a rig turned all the way around.
- **`v`** -- world height, normalized 0.0-1.0 over the selection. Height is *not* discovered by PCA: gravity is a real shared axis, so up always stays up and a dipping truss reads as a dip rather than as a sweep.

Degenerate rigs: a selection with no horizontal spread (a single vertical stack) falls back to `u` = normalized index; a flat selection with no height spread gets `v` = 0.5 everywhere.

Both are computed over the current **selection**, like every other spatial attribute -- two different selections in the same venue can therefore have different `u` axes. A venue-wide basis with per-group overrides is future work.

Implementation: `rig_uv` in `backend/src/eval/ops/spatial.rs`.
...
File: `backend/src/node_graph/circle_fit.rs`
