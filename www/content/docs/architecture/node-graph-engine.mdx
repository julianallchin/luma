...
The node graph engine evaluates pattern graphs to produce time-varying light output for fixtures. It lives in `backend/src/node_graph/`.

## Execution Model

File: `backend/src/node_graph/executor.rs`
...
Defined in `backend/src/node_graph/state.rs`:
...
File: `backend/src/node_graph/nodes/mod.rs`
...
Each node receives a `NodeExecutionContext` (defined in `backend/src/node_graph/node_execution_context.rs`) with:
...
File: `backend/src/node_graph/nodes/selection.rs`
...
1. Selection expressions are parsed into an AST by the expression parser in `backend/src/services/groups.rs`
