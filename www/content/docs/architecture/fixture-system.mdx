...
File: `backend/src/fixtures/parser.rs`
...
File: `backend/src/fixtures/layout.rs`
...
The selection system (`backend/src/node_graph/nodes/selection.rs`) converts head positions to meters and applies the fixture's 3D rotation (Euler angles: X roll, Y pitch, Z yaw) to produce global coordinates.
...
The fixture library path is resolved at startup from the source tree or the host resource directory (`resources/fixtures/2511260420`). The directory structure follows the QLC+ convention:
