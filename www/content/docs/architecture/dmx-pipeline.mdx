...
The `PrimitiveState` struct (defined in `backend/src/models/universe.rs`) represents a single fixture or head at a single moment in time:
...
File: `backend/src/fixtures/engine.rs`
...
File: `backend/src/render_engine.rs`
...
1. **Native visualizer** samples the shared render engine for its current frame
2. **ArtNetManager** for DMX broadcast to physical hardware

### `render_frame()`

The `render_frame()` function (in `backend/src/engine/mod.rs`) samples each primitive's Series by finding the nearest sample via linear scan (closest time distance).
