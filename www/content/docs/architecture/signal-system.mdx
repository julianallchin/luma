...
The `Signal` struct is the fundamental data type in Luma's graph engine. Defined in `backend/src/models/node_graph.rs`:
...
A `Series` is the time-series representation for a single fixture's channel after a Signal is converted by an apply node. Defined in `backend/src/models/node_graph.rs`:
...
At render time, the compositor uses binary search (`partition_point`) for O(log n) lookup of the nearest sample, with optional linear interpolation between adjacent samples. The `sample_series()` function in `backend/src/compositor.rs` handles this lookup.
