...
- **Venue data** (venue-specific): Patched fixtures (DMX addresses, 3D positions, rotations), fixture groups, scores and annotations (pattern placements on track timelines)

Venues are rows in a `venues` table. Fixtures, groups, and scores link to venues via foreign keys. Library data syncs automatically and bidirectionally to Supabase for cloud backup and community sharing.
...
src-tauri/src/
  ...
  models/                   # Data structures (serde + ts-rs)
    ...
    groups.rs               # FixtureGroup, GroupMember
  ...
  services/                 # Business logic
    ...
    groups.rs               # Group management, selection expression parsing, spatial queries
  ...
  sync/                     # Automatic bidirectional Supabase sync
    orchestrator.rs         # Full sync cycles (pull then push)
    push.rs                 # Background push loop, pending ops drain
    pull.rs                 # Delta pull with per-user timestamps
    files.rs                # Album art, waveform, binary asset sync
    traits.rs               # Syncable trait, generic sync registry
