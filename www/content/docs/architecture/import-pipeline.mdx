...
File: `backend/src/services/tracks.rs`
...
Progress reaches the native UI through the dispatch event sink (`track-import-state`). The preprocessing scheduler owns in-flight claims and identity-scoped task leases to prevent duplicate work and cancel analysis across identity changes.
...
File: `backend/src/python_env.rs`
...
Files: `backend/crates/stagelinq/src/`
